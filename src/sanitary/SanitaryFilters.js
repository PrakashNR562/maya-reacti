...
const SanitaryFilters = function ({ categoryAttached }) {
  const {
  ...
  const categories = getUniqueValues(all_sanitary, "category");
  const companies = getUniqueValues(all_sanitary, "company");
  let colors = getUniqueValues(all_sanitary, "color");

  const types = getUniqueValues(all_sanitary, "type");
  ...
  return <h1>Sanitary Filters</h1>;
};

export default SanitaryFilters;
